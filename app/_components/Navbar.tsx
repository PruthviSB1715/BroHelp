...
import { Wallet, LogOut, User, ChevronDown, LayoutDashboard, ListTodo } from "lucide-react";
...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 w-full z-50 border-b border-white/5 backdrop-blur-xl bg-[#0a0a0f]/80" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
      ...
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <Link href="/dashboard" className="flex items-center gap-2">
          ...
          <span style={{ fontFamily: "'Syne', sans-serif" }} className="font-bold text-lg text-white">TaskHelper</span>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-6 text-sm text-white/40">
          ...
          <Link href="/wallet" className="nav-link flex items-center gap-1.5">
            <Wallet size={14} />
            <span className="text-white font-medium">{user?.credits ?? 0}</span> credits
          </Link>
        </div>
        ...
        <NotificationBell/>
          ...
          >
            ...
            <ChevronDown size={13} className={`text-white/30 transition-transform ${dropdownOpen ? "rotate-180" : ""}`} />
          </button>
          ...
          {dropdownOpen && (
            <>
              ...
              {/* Dropdown */}
              <div className="dropdown absolute right-0 top-12 w-52 rounded-2xl p-1.5 z-50">
                ...
                <Link
                  ...
                  className="dropdown-item flex items-center gap-2.5 rounded-xl px-3 py-2.5 text-sm text-white/60 hover:text-white"
                >
                  <Wallet size={14} /> Wallet · <span className="text-indigo-400 font-medium">{user?.credits ?? 0}</span>
                </Link>
